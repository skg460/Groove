<?php
	...
	require ('connect.php');
     $fmsg="";
	if(isset($_POST) && !empty($_POST)&&isset($_POST['email'])&&isset($_POST['username'])&&isset($_POST['password']))
	{
		$_SESSION['username'] = $username = mysqli_real_escape_string($connection,$_POST['username']);
		$_SESSION['email'] = $email = mysqli_real_escape_string($connection,$_POST['email']);
		$_SESSION['password'] = $password = $_POST['password'];

		$query = "SELECT * FROM users WHERE email='$email' ";
		$result = mysqli_query($connection,$query);
      if (!filter_var($email, FILTER_VALIDATE_EMAIL)) 
        $fmsg="Incorrect email-id";
      else if( !preg_match("#[0-9]+#", $password) ) {
           $fmsg= "Password must include at least one number! ";
            }
            else if( !preg_match("#[a-z]+#", $password) ) {
              $fmsg= "Password must include at least one letter! ";
             }

            else if( !preg_match("#[A-Z]+#", $password) ) {
           $fmsg= "Password must include at least one CAPS! ";
              }

         else if( !preg_match("#\W+#", $password) ) {
          $fmsg= "Password must include at least one symbol! ";
          }
        else if(strlen($password)<8)
        {
        	$fmsg="Password must be atleast 8 characters long!";
        }
		else if(mysqli_num_rows($result)>0)
		{
			$fmsg="email-id already exist!";
		}
		else
